...
import { handleExtensionRequest } from "./extension";
...
function serveStaticAsset(distDir: string, path: string): Response | null {
...
function createSPAHandler(distDir: string): Response {
...
export function createServer(options: ServerOptions = {}): Server {
  ...
  const serveWebUI = options.serveWebUI ?? process.env.SERVE_WEBUI !== "false";
  ...
  if (serveWebUI) {
    if (isProduction) {
      ...
      routes["/*"] = async (request: Request) => {
        ...
        const staticResponse = serveStaticAsset(distDir, url.pathname);
        if (staticResponse) {
          return staticResponse;
        }
        return createSPAHandler(distDir);
      };
    } else {
      routes["/auth/*"] = authHandler;
