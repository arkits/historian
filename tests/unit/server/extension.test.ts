...
describe("Extension Tests", () => {
  ...
  describe("handleImport - Authentication", () => {
    ...
    it("should update lastUsedAt when authenticating", async () => {
      ...
      expect(new Date(keyAfter!.lastUsedAt!).getTime()).toBeGreaterThan(Date.now() - 5000);
    });
  });
  ...
  describe("handleImport - Import Logic", () => {
    ...
    it("should import single history item", async () => {
      ...
      expect(items[0]?.content).toEqual({ url: "https://example.com", title: "Example" });
    });
        ...
        body: JSON.stringify({
          items: [
            ...
            },
            {
              ...
              type: "video",
              contentId: "content-2",
              content: { url: "https://example.com/video", title: "Example Video" },
            },
            {
              timelineTime: new Date(Date.now() - 2000).toISOString(),
              type: "page",
              contentId: "content-3",
              content: { url: "https://example.com/page3", title: "Example 3" },
            },
          ],
        }),
      });
    ...
    it("should correctly map all fields from items", async () => {
      ...
      expect(new Date(items[0]!.timelineTime).toISOString()).toBe(new Date(timelineTime).toISOString());
      expect(items[0]?.type).toBe("page");
      expect(items[0]?.contentId).toBe("content-1");
      expect(items[0]?.content).toEqual({ url: "https://example.com", title: "Example" });
